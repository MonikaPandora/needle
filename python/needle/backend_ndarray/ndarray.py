...
class BackendDevice:
    ...
    def ones(self, *shape, dtype="float32"):
        return NDArray(np.ones(shape=shape).astype(dtype), device=self)
...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        if not isinstance(new_shape, tuple):
            raise ValueError(f'Given shape is not a tuple')
        flag = -1
        p_new = 1
        for i in range(len(new_shape)):
            if new_shape[i] == 0:
                raise ValueError('dim size should not be zero')
            if new_shape[i] < 0:
                if flag != -1:
                    raise ValueError('more than one dim be auto inferred')
                else:
                    flag = i
            else:
                p_new *= new_shape[i]
        ...
        if flag != -1:
            p = prod(self.shape)
            temp_lst = list(new_shape)
            if p % p_new:
                raise ValueError('incompatible new shape')
            temp_lst[flag] = p // p_new
        elif prod(new_shape) != prod(self.shape):
            raise ValueError(f'New shape [{new_shape}] incompatible with current {self.shape}')
        return NDArray.make(shape=new_shape, strides=NDArray.compact_strides(new_shape), 
                            device=self.device, handle=self.compact()._handle, offset=0)
        ### END YOUR SOLUTION
    ...
    def permute(self, new_axes):
        ...
        if len(new_axes) != len(self.shape):
            raise ValueError('number of dimensions of new_axes does not match the length of the desired ordering of dimensions')
        ...
        for axis in new_axes:
            if axis in dup:
                raise RuntimeError('permute(): dup axes is not allowed')
            dup.append(axis)
            new_shape.append(self.shape[axis])
            new_strides.append(self.strides[axis])
        
        return NDArray.make(shape=tuple(new_shape), strides=tuple(new_strides),
    ...
    def broadcast_to(self, new_shape):
        ...
        ### BEGIN YOUR SOLUTION
        assert len(new_shape) >= len(self._shape), "Broadcasted new shape should be longer"
        # pad self._shape with 1 if new_shape is longer, tail part should matched or can be broadcasted
    ...
    def __getitem__(self, idxs):
        ...
        for i in range(self.ndim):
            sl = idxs[i]
            offset += sl.start * self.strides[i]
            new_strides.append(self.strides[i] * sl.step)

            s = (sl.stop - sl.start) // sl.step
            if (sl.stop - sl.start) % sl.step:
                s += 1
            new_shape.append(s)
    ...
    def reduce_view_out(self, axis, keepdims=False):
        ...
        if axis is None:
            ...
            out = NDArray.make((1,) if not keepdims else tuple([1] * len(self.shape)), device=self.device)

        else:
            if isinstance(axis, (tuple, list)):
                # assert len(axis) == 1, "Only support reduction over a single axis"
                # axis = axis[0]
                all_axes = list(range(self.ndim))
                axes = [all_axes[ax] for ax in axis]
            else:
                axes = [axis]
    ...
    def flip(self, axes):
        ...
        for axis in axes:
            ...
            new_strides[axis] *= -1
        return self.make(shape=self.shape, strides=tuple(new_strides), 
    ...
    def pad(self, axes):
        ...
        """
        ### BEGIN YOUR SOLUTION
        assert len(axes) == len(self.shape), "need to specify padding on all dims"
        new_shape = list(self.shape)
        slices = [slice(None)] * len(new_shape)
        for i, amount in enumerate(axes):
            new_shape[i] += amount[0] + amount[1]
            slices[i] = slice(amount[0], new_shape[i] - amount[1])
        
        ret = self.device.full(tuple(new_shape), 0)
