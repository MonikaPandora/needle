...
class Tensor(Value):
    ...
    def __repr__(self):
        return "needle.Tensor(" + str(self.realize_cached_data()) + f", device={self.device}" + ")"
...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        ...
        if node.op:
            back = node.op.gradient_as_tuple(node.grad, node)
        for input in node.inputs:
            if input not in node_to_output_grads_list.keys():
                node_to_output_grads_list[input] = []
            
            node_to_output_grads_list[input].append(back[node.inputs.index(input)])
    ### END YOUR SOLUTION
